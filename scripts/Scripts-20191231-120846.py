...
from keras.layers import TimeDistributed, Bidirectional, GRU, LSTM, Dense, Activation, Dropout, Flatten, Conv2D, MaxPooling2D, DepthwiseConv2D, AveragePooling2D, SeparableConv2D, Reshape, ZeroPadding2D
...
def Preprocessing(data, events, downsampling=0, targets=None, filter_feature_in=None, sort=False, reshape="None", norm=False, spectrum=True, low_pass=0):
	...
	if low_pass != 0:
		...
		for i, single_channel in enumerate(data):
			for j, single_epoch in enumerate(single_channel):
				output[i][j] = BasicDataProcess.LowPassFilter(low_pass, single_epoch)
...
for sbj_no in range(1, 16):
	...
	model.add(Reshape((1,8,350), input_shape=(8,350)))
	model.add(ZeroPadding2D(padding=(0, 32), data_format='channels_first'))
	# 1st Convolutional Layer
	model.add(Conv2D(filters=f1, kernel_size=(1,125), padding="valid", activation='linear', data_format='channels_first'))
	...
	model.add(SeparableConv2D(filters=f2, kernel_size=(1,16), padding='valid', activation='linear', data_format='channels_first'))
	model.add(BatchNormalization(axis=1))
	model.add(Activation('elu'))	
	# Pooling 
	...
	model.add(Bidirectional(GRU(64, activation='elu', return_sequences=True)))
	model.add(Bidirectional(GRU(64, activation='elu', return_sequences=True)))
	model.add(BatchNormalization(axis=1))
	model.add(Activation('elu'))
	...
	model.add(Flatten())
	# Output Layer
	model.add(Dense(2, activation='linear', kernel_constraint=max_norm(0.25)))
